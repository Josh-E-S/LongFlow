...
3. **Verify frame alignment before wiring anything.** Cached hidden states `[B, T, 2048]` and acoustic latents `[B, T, 64]` must share the same T. A misalignment here reproduces the April 7 failure signature (sharp but unintelligible audio).
