...
5. **Keep adapters thin** (finding N5). The flow head is ~15M params, no attention. Don't add capacity to fix a conditioning problem. **[AMENDED 2026-08-18, Josh's call:** conditioning is long since acquitted (review §3.4) and the speed story is carried by batching, not head size — the contribution is the method + the measured quality-vs-size-vs-NFE tradeoff curve. Capacity increases (width 960 → larger, more codes, etc.) are now PERMITTED, but only after the cheap no-param levers (teacher-polish, combined clean pool, MeanFlow objective) have been tried at the current size — otherwise gains can't be attributed. The rule's spirit survives: capacity moves last, never to mask a diagnosable defect.**]**
